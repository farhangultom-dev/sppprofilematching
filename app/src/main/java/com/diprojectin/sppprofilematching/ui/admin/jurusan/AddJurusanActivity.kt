...
class AddJurusanActivity : AppCompatActivity() {
    private lateinit var binding: ActivityAddJurusanBinding
    private lateinit var dialogConfirm: Dialog
    private lateinit var loadingDialog: Dialog

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAddJurusanBinding.inflate(layoutInflater)
    ...
    private fun sendData() {
        ...
        loadingDialog = DialogLoading(this@AddJurusanActivity,
        ...
        val call = apiClient?.addJurusan(namaJurusan)
        call?.enqueue(object: Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                loadingDialog.dismiss()
                if(!response.isSuccessful){
                    Toast.makeText(this@AddJurusanActivity, "Terjadi kesalahan, lakukan beberapa saat", Toast.LENGTH_SHORT).show()
                    return
                }

                if(response.isSuccessful){
                    if (response.body()?.success == true){
                        Toast.makeText(this@AddJurusanActivity, "Berhasil menambahkan jurusan", Toast.LENGTH_SHORT).show()

                        val intent = Intent(this@AddJurusanActivity, MasterJurusanActivity::class.java)
                    ...
                    if (response.body()?.success == false){
                        Toast.makeText(this@AddJurusanActivity, response.body()?.message, Toast.LENGTH_SHORT).show()
                        return
                    }
                }
            }
            ...
            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                loadingDialog.dismiss()
                Toast.makeText(this@AddJurusanActivity, t.message, Toast.LENGTH_SHORT).show()
            }
