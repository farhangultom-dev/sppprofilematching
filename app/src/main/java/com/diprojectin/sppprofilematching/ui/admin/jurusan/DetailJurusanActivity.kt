...
class DetailJurusanActivity : AppCompatActivity() {
    ...
    private fun initData() {
        namaJurusan = intent.getStringExtra("namaJurusan").toString()
        idJurusan = intent.getStringExtra("id").toString()

    }

    private fun initView() = with(binding) {
        tvNamaJurusan.text = namaJurusan

        appBar.tvTitleHeader.text = "Detail Jurusan"
        ...
        btnDelete.setOnClickListener {
            ...
            subtitle.text = "Apakah anda yakin akan hapus data jurusan ini?"
            btnSimpan.text = "Ya"
        ...
        btnEdit.setOnClickListener {
            val intent = Intent(this@DetailJurusanActivity, EditJurusanActivity::class.java)
            intent.putExtra("id", idJurusan)
            intent.putExtra("namaJurusan", namaJurusan)
    ...
    private fun deleteJurusan() {
        loadingDialog = DialogLoading(this@DetailJurusanActivity,
            "Mohon tunggu, sedang menghapus data kelas",false).build()

        loadingDialog.show()
        val apiClient = ApiClient.client(this)?.create(ApiInterface::class.java)
        val call = apiClient?.deleteJurusan(idJurusan)
        call?.enqueue(object: Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                loadingDialog.dismiss()
                if(!response.isSuccessful){
                    Toast.makeText(this@DetailJurusanActivity, "Terjadi kesalahan, lakukan beberapa saat", Toast.LENGTH_SHORT).show()
                    return
                }

                if(response.isSuccessful){
                    if (response.body()?.success == true){
                        Toast.makeText(this@DetailJurusanActivity, "Berhasil menghapus jurusan", Toast.LENGTH_SHORT).show()
                        val intent = Intent(this@DetailJurusanActivity, MasterJurusanActivity::class.java)
                    ...
                    if (response.body()?.success == false){
                        Toast.makeText(this@DetailJurusanActivity, response.body()?.message, Toast.LENGTH_SHORT).show()
                        return
                    }
                }
            }
            ...
            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                loadingDialog.dismiss()
                Toast.makeText(this@DetailJurusanActivity, t.message, Toast.LENGTH_SHORT).show()
            }
