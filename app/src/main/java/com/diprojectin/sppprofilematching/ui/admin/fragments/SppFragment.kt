...
class SppFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentSppBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun getRiwayat() = with(binding) {
        ...
        val call = apiClient?.getRiwayatAdmin()
        call?.enqueue(object: Callback<RiwayatTransactionsResponse> {
            override fun onResponse(call: Call<RiwayatTransactionsResponse>, response: Response<RiwayatTransactionsResponse>) {
                ...
                if(!response.isSuccessful){
                    android.widget.Toast.makeText(requireActivity(), "Terjadi kesalahan, lakukan beberapa saat", android.widget.Toast.LENGTH_SHORT).show()
                    return
                }
                ...
                if(response.isSuccessful){
                    ...
                    if (response.body()?.success == false){
                        android.widget.Toast.makeText(requireActivity(), response.body()?.message, android.widget.Toast.LENGTH_SHORT).show()
                        return
                    }
                }
            }
            ...
            override fun onFailure(call: Call<RiwayatTransactionsResponse>, t: Throwable) {
                loadingDialog.dismiss()
                android.widget.Toast.makeText(requireActivity(), t.message, android.widget.Toast.LENGTH_SHORT).show()
            }
